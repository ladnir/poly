...
namespace poly2
{
    ...
    public:
        ...
        template<typename U>
        static constexpr bool is_acceptable = std::is_convertible<U*, T*>::value;

        // SFINAE predicate for local storage. 
        template<typename U>
        static constexpr bool is_small = (sizeof(U) <= storage_size);
        ...
        template<typename U, size_t other_size,
            typename Enabled =
            typename std::enable_if<is_acceptable<U>>::type
        >
        ...
        template<typename U, size_t other_size>
        typename std::enable_if<is_acceptable<U>,
            this_type&>::type
        ...
        template<typename U>
        typename std::enable_if<is_acceptable<U>,                    // U* can cast as T* and should be inlined.
            this_type&>::type // return *this
            operator=(U&& u)
        {
            destruct();
            if constexpr(sizeof(U) <= storage_size)
        ...
        template<typename U>
        typename std::enable_if<is_acceptable<U>>::type                                // return void
            reset(U* u)
        {
            destruct();
